...
// CHƯA LÀM LẠI
         ...
         public function searchTask($keyword){
            $search = "%{$keyword}%";
            $sql = parent::$connection->prepare('SELECT * FROM `task` WHERE name_task  LIKE ? ');
            $sql->bind_param('s', $search);
            return parent::select($sql);
}
      
      
            public function panigination($idPage,$totalTaskPages){
               $sql = parent::$connection->prepare(" SELECT * FROM `task` LIMIT $idPage,$totalTaskPages");
               return parent::select($sql);
}
